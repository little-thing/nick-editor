...
export const findRow = (selection: Selection) =>
  findParentNode((node) => node.type.spec.tableRole && node.type.spec.tableRole === 'row')(selection);
export const listRows = (table: Node) => {
  return findChildren(table, (node) => node.type.spec.tableRole && node.type.spec.tableRole === 'row');
};
...
export const findTableWrapper = (selection: Selection) => {
  // 首先找到table节点
  const tableWrapper = findParentNode(
    (node) => node.type.spec.tableRole && node.type.spec.tableRole === 'tableWrapper'
  )(selection);
